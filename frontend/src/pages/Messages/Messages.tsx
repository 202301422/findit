...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true)
      try {
        const data = await getConversations(1, 20)
        if (!cancelled) {
  ...
  const sentinelRef = useInfiniteScroll({ hasMore, loading: loadingMore, onLoadMore: handleLoadMore })
          ...
          <>
            ...
            {loadingMore && (
              Array.from({ length: 2 }).map((_, idx) => (
                <div key={`more-${idx}`} className="flex items-center gap-3 p-4 animate-shimmer">
                  <div className="w-11 h-11 rounded-full bg-[var(--bg-tertiary)] shrink-0" />
                  <div className="flex-1 space-y-2">
                    <div className="h-4 bg-[var(--bg-tertiary)] rounded-xs w-1/3" />
                    <div className="h-3 bg-[var(--bg-tertiary)] rounded-xs w-2/3" />
            ...
            {/* All loaded */}
            {!hasMore && !loadingMore && conversations.length > 0 && (
              <p className="text-center text-xs text-[var(--text-tertiary)] font-medium py-3">
