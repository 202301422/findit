...
import { Sparkles, X, Send, Trash2, ArrowRight, Layers, GripHorizontal, Mic, MicOff, Eye } from 'lucide-react'
...
export default function AssistantPanel({ isOpen, onClose }: AssistantPanelProps) {
  ...
  const handleMicClick = () => {
    ...
    const SpeechRecognition =
      (window as unknown as Record<string, SpeechRecConstructor>).SpeechRecognition ||
      (window as unknown as Record<string, SpeechRecConstructor>).webkitSpeechRecognition
        ...
        <>
          ...
          >
            ...
            <div className="p-3 bg-[var(--surface-card)] border-t border-[var(--border-primary)] shrink-0">
              <div className="relative flex items-end gap-2 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-[var(--radius-xl)] p-2 focus-within:ring-2 focus-within:ring-[var(--color-primary-500)]/30 focus-within:border-[var(--color-primary-500)] transition-all">
                <textarea
                  value={input}
                  ...
                  rows={1}
                  maxLength={1000}
                  className="w-full bg-transparent text-xs sm:text-sm text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] resize-none focus:outline-none max-h-24 px-1 py-1 scrollbar-none"
                />

                <button
                  type="button"
                  onClick={handleMicClick}
                  title={isListening ? 'Stop listening' : 'Speak to GetIt'}
                  className={clsx(
                    'p-2 rounded-full transition-all cursor-pointer shrink-0 shadow-xs',
                    isListening
